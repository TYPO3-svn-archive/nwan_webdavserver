...
class tx_metaftpd_t3io 
{
	...
	{
		if (!$forceT3 && $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['meta_ftpd']['natural_file_names']) 
			$ret=$row['header'];
		else
	...
	function T3GetCTypeFile(&$fileinfo) 
	{
		...
		if (is_array($conf['plugin.']['tx_metaftpd.'][$fileinfo['ext'].'.']['put.'])) 
		{
	  		...
	  		if ($extConf['headerField'])
	  		{
	 			$contentDataArray[$extConf['headerField']]=$fileinfo['filename'];
	  		}
	  		
	 		if ($extConf['ctypeField'] && $extConf['ctype'])
	 		{ 
	  			$contentDataArray[$extConf['ctypeField']]=$extConf['ctype'];
	 		}
	  		
	  		if ($extConf['listTypeField'] && $extConf['list_type'] && $extConf['ctype']=='list')
	  		{	  			
	  			$contentDataArray[$extConf['listTypeField']]=$extConf['list_type'];
	  		} 
	...
	function T3FileFieldCopy($fileinfo,$field,&$contentDataArray) 
	{
		...
		if ($field) 
		{
			...
			if ($fieldCFG['uploadfolder']) 
			{
				if ($field!='tx_metaftpd_ftpfile') 
				{
					$dir=$this->T3CleanFilePath($this->CFG->T3PHYSICALROOTDIR.'/'.$fieldCFG['uploadfolder']);
				}
				else 
				{
				...
				if ($name) 
				{
					$contentDataArray[$field]=$this->T3GetFileName($name);
				}
			}
		}
	}
	...
	function T3FlexFileCopy($fileinfo,&$flexDS,$flexconf,&$flexArray) 
	{
		...
		if ($sheet && $field) 
		{
			...
			if ($fieldCFG['uploadfolder']) 
			{
				...
				if ($name) 
				{
					$flexArray['data'][$sheet]['lDEF'][$field]['vDEF']=$this->T3GetFileName($name);
				}
			}
		}
	}
