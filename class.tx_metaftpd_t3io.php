...
function T3Authenticate($username,$password,$crypt="md5") {
	$this->metaftpd_devlog(1,"====== T3Authenticate : $username ",'meta_t3io','T3Authenticate');
	$auth=false;
	...
	switch ($crypt) {
	 ...
	 //$this->metaftpd_devlog(4,$this->CFG->T3DB->SELECTquery('*','be_users',"be_users.username='$username' and be_users.password='$pass'"),'meta_t3io');
	 if ($res) {
      ...
      $FILEMOUNTS=$this->BEUSER->groupData['filemounts'];
		  ...
		  $this->metaftpd_devlog(2,"T3Authenticate, user : $this->user_uid , FILEMOUNTS : ".serialize($FILEMOUNTS).", WEBMOUNTS : $WEBMOUNTS",'meta_t3io','T3Authenticate');
      $T3EXTFILE=t3lib_div::makeInstance('t3lib_extFileFunctions');
		...
		$this->metaftpd_devlog(2,"T3Authenticate : PB DB",'meta_t3io','T3Authenticate');
	}
	$this->metaftpd_devlog(1,"====== T3Authenticate end : $auth ",'meta_t3io','T3Authenticate');
  return $auth;  
}
...
function T3ListDir($path) {
	...
	$this->metaftpd_devlog(1,"============ T3ListDir start :".$path,'meta_t3io','T3ListDir');//." fileinfo : ".serialize($fileinfo)
	$list=array();
	// Is Path a webmount ?			
	if ($fileinfo['isWebmount']) {
		...
		if ($fileinfo['level']==1) {
			...
			$webmounts=$this->BEUSER->groupData['webmounts'];
			$this->metaftpd_devlog(1,"============ T3ListDir webmounts :".$webmounts,'meta_t3io','T3ListDir');
			//$parr[]=array();
			if (strlen(trim($webmounts))>0) {
				$pids=t3lib_div::trimexplode(',',$webmounts);
				foreach($pids as $pid) {
					$this->metaftpd_devlog(1,"============ T3ListDir webmounts pid:".$pid,'meta_t3io','T3ListDir');
					if ($pid==0) {
						$parr[]=array('uid'=>0, 'title'=>'Root');
				  } else	$parr[]=$this->CFG->T3PAGE->getPage($pid);
				}
			}
		} else {
		...
		} else if ($path=='/'.T3_FTPD_FILE_ROOT.'/') {
			$filemounts=$this->BEUSER->groupData['filemounts'];
